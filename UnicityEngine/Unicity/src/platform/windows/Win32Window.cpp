...
namespace u_engine {
	namespace graphics {
		...
		bool Window::platformInit()
		{
			...
			{
				// TODO: Handle error
				UNICITY_ERROR("Could not register Win32 class!");
				return false;
			}
			...
			if (!hWnd)
			{
				UNICITY_ERROR("Could not create window!");
				return false;
			}
			...
			if (pixelFormat)
			{
				...
				{
					UNICITY_ERROR("Failed setting pixel format!");
					return false;
				}
			}
			else
			{
				UNICITY_ERROR("Failed choosing pixel format!");
				return false;
			}
			...
			if (hrc)
			{
				...
				{
					UNICITY_ERROR("Failed setting OpenGL context!");
					return false;
				}
			}
			else
			{
				UNICITY_ERROR("Failed creating OpenGL context!");
				return false;
			}
		...
		void Window::setMouseCursor(int cursor) {
			...
			if (cursor == UE_NO_CURSOR) {

				SetCursor(UE_NO_CURSOR);
		...
		void key_callback(Window* window, int key, UE_uint message)
		{
